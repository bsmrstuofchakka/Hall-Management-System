...
class MealController extends Controller
{
    ...
    public function request_for_token(Request $request)
    {
        ...
        if ($token_sell_option->status == 0) {
            ...
            $already_requested_num = App\Meal::where([['student_id', '=', $studentId], ['date', '>=', date("d-m-Y",strtotime('tomorrow'))]])->count();
            ...
            if ($already_requested_num > 0) {
                ...
                if ($morning_meal_quantity==0&&$launch_meal_quantity==0&&$dinner_meal_quantity==0)
                {
                    return redirect('buy_meal')->with('error_message', 'You Must Select At least One Meal in Any Meal Period');
                }
                else
                {
...
//                    {
                    ...
                    $student->save();
                    return redirect('buy_meal')->with('success_message', 'Your Token Buying Request Has Placed successfully for date:'.$date);
//                    }
                }
    ...
    public function direct_token_list(){
        $date_n=date("h-i-s");
//        echo $date_n;
        $date_am_pm=date('a');
        $date=null;
        if ($date_n>"8-00-00"&&$date_am_pm=="pm"){
            $date=date('d-m-Y',strtotime("tomorrow"));
        }
        else{
            $date=date('d-m-Y');
        }
        $tokens=$tokens = App\Meal::where([['date', '=', $date]])->get();
        return view('token_list',['tokens'=>$tokens]);
    }
